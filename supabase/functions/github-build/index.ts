...
ENTEOF
      ...
      - name: Build Xcode archive
        run: |
          ...
          SPM_DIR="\$(xcodebuild -project App.xcodeproj -scheme App -showBuildSettings 2>/dev/null | grep BUILD_DIR | head -1 | sed 's/.*= //' | sed 's|/Build/.*|/SourcePackages|')"
          echo "SPM directory: \$SPM_DIR"
          find "\$(pwd)" "\$SPM_DIR" ~/Library/Developer/Xcode/DerivedData -path "*/OSBarcodeLib*OSBARCScannerView.xcassets" -type d 2>/dev/null | sort -u | while read XCASSET; do
