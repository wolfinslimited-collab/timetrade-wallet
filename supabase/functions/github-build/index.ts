      ...
      - name: Remove alpha channel from all icons
        run: |
          ...
          for IMG in "\$ICON_DIR"/*.png; do
            if [ -f "\$IMG" ]; then
              sips -s format png -s formatOptions 100 "\$IMG" --out "\$IMG.tmp"
              sips -s hasAlpha false "\$IMG.tmp" --out "\$IMG"
              rm -f "\$IMG.tmp"
      ...
      - name: Set iOS build number
        run: |
          ...
          if [ -f "\$PBXPROJ" ]; then
            ...
            sed -i '' "s/MARKETING_VERSION = [^;]*;/MARKETING_VERSION = 1.0.\${GITHUB_RUN_NUMBER};/g" "\$PBXPROJ"
          fi
  ...
  try {
    ...
    switch (body.action) {
      ...
      case "push-workflow": {
        ...
        if (!WORKFLOW_MAP[platform]) throw new Error(`No workflow for platform: ${platform}`);

        return new Response(
          JSON.stringify({
            success: true,
            message: `Workflows are managed in the repo at .github/workflows/. Push changes via git.`,
