...
jobs:
  build-ios:
    ...
    steps:
      ...
      - name: Configure custom URL scheme (OAuth return)
        run: |
          PLIST="ios/App/App/Info.plist"
          /usr/libexec/PlistBuddy -c "Delete :CFBundleURLTypes" "\$PLIST" 2>/dev/null || true
          /usr/libexec/PlistBuddy -c "Add :CFBundleURLTypes array" "\$PLIST"
          /usr/libexec/PlistBuddy -c "Add :CFBundleURLTypes:0 dict" "\$PLIST"
          /usr/libexec/PlistBuddy -c "Add :CFBundleURLTypes:0:CFBundleURLName string com.wallet.ai" "\$PLIST"
          /usr/libexec/PlistBuddy -c "Add :CFBundleURLTypes:0:CFBundleURLSchemes array" "\$PLIST"
          /usr/libexec/PlistBuddy -c "Add :CFBundleURLTypes:0:CFBundleURLSchemes:0 string com.wallet.ai" "\$PLIST"
          echo "CFBundleURLTypes configured for com.wallet.ai"
...
jobs:
  build-android:
    ...
    steps:
      ...
      - name: Bump minSdkVersion to 26
        run: |
          sed -i 's/minSdkVersion = [0-9]*/minSdkVersion = 26/' android/variables.gradle
          echo "minSdkVersion bumped to 26"
      ...
      - name: Set Android build number
        run: |
          BASE_OFFSET=1000
          VERSION_CODE=\$(( BASE_OFFSET + GITHUB_RUN_NUMBER * 100 + \${GITHUB_RUN_ATTEMPT:-1} ))
      ...
      - name: Patch build.gradle for release signing
        run: |2
          ...
          if not gradle.exists():
              print("FATAL: android/app/build.gradle not found")
              sys.exit(1)

          src = gradle.read_text()

          if "signingConfig signingConfigs.release" in src:
              print("build.gradle already contains signing config, skipping")
              sys.exit(0)
          ...
          if "def keystorePropertiesFile" not in src:
              if "android {" in src:
                  ...
                  print("FATAL: 'android {' block not found")
                  sys.exit(1)
          ...
          if "signingConfigs {" not in src:
              if "    buildTypes {" in src:
                  ...
                  print("FATAL: 'buildTypes' block not found")
                  sys.exit(1)
